...
let
  cfg = config.mySystem.agenix.sshKeys;
in
{
  options.mySystem.agenix.sshKeys = with lib; {
    enable = mkEnableOption "sshKeys";
  };

  config = lib.mkIf (config.mySystem.enable && config.mySystem.agenix.enable && cfg.enable) {
