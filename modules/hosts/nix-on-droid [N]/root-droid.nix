{ inputs, ... }:
{
  flake.modules.nixOnDroid.root-droid =
    ...
    {
      # ── KEY CHANGE ────────────────────────────────────────────────────────
      # environment.files.login is readOnly = true in nix-on-droid's login
      # module, so we cannot assign it at all (mkForce doesn't help).
      # Instead, overwrite the symlink imperatively after activation completes.
      build.activationAfter.replace-login = ''
        echo "Replacing login with custom root-aware login..."
