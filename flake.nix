{
  ...
  outputs = inputs @ { self, nixpkgs, nixos-hardware, home-manager, nixos-nvidia-vgpu, ...}:
    let
    ...
    in {
      nixosConfigurations = (                                 # Location of the available configurations
        import ./hosts {                                      # Imports ./hosts/default.nix
          inherit (nixpkgs) lib;
          inherit inputs user location system nixos-hardware home-manager nixos-nvidia-vgpu;            # Also inherit home-manager so it does not need to be defined here.
        }
      );
